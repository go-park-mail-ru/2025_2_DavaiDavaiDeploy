...
func testContextWithUsersUser(user models.User) context.Context {
	ctx := testContext()
	return context.WithValue(ctx, users.UserKey, user.ID)
}
...
func TestGetPromoFilm(t *testing.T) {
	...
	handler := NewFilmHandler(mockClient, nil)
...
func TestGetFilms(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			mockSetup: func() {
				...
				// Создаем 12 фильмов (count + 1 + 1 дополнительный для курсора)
				for i := 0; i < 12; i++ {
					createdAtStr := time.Now().Format("2006-01-02 15:04:05.999999999 -0700 MST")
				...
				mockClient.EXPECT().
					GetFilms(gomock.Any(), &gen.GetFilmsRequest{
						Count: 12, // count + 1 для курсора
					}).
					Return(&gen.GetFilmsResponse{Films: grpcFilms}, nil)
			},
			expectedStatus: http.StatusOK,
			expectBody:     true,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectBody && tt.expectedStatus == http.StatusOK {
				var decoded []models.MainPageFilm
				err := json.Unmarshal(rec.Body.Bytes(), &decoded)
				assert.NoError(t, err)
				// В зависимости от теста, может быть разное количество фильмов
				if strings.Contains(tt.name, "Success with cursor") {
					assert.Equal(t, 10, len(decoded)) // Меньше чем count+1
				} else if strings.Contains(tt.name, "Success") {
					assert.Equal(t, 11, len(decoded)) // count (12) - 1 для курсора
				}
			}
		})
	}
			...
			mockSetup: func() {
				...
				for _, feedback := range expectedFeedbacks {
					grpcFeedbacks = append(grpcFeedbacks, &gen.FilmFeedback{
						...
						CreatedAt:  feedback.CreatedAt.Format(TimeLayout),
						UpdatedAt:  feedback.UpdatedAt.Format(TimeLayout),
				...
				mockClient.EXPECT().
					...
					Return(&gen.SendFeedbackResponse{
						Feedback: &gen.FilmFeedback{
							...
							Text:       &text,
							Rating:     9,
							CreatedAt:  expectedFeedback.CreatedAt.Format(TimeLayout),
							UpdatedAt:  expectedFeedback.UpdatedAt.Format(TimeLayout),
							UserLogin:  "testuser",
							UserAvatar: "/avatars/test.jpg",
							IsMine:     true,
						},
				...
				mockClient.EXPECT().
					...
					Return(&gen.SetRatingResponse{
						Feedback: &gen.FilmFeedback{
							...
							Text:       nil,
							Rating:     8,
							CreatedAt:  expectedRating.CreatedAt.Format(TimeLayout),
							UpdatedAt:  expectedRating.UpdatedAt.Format(TimeLayout),
							UserLogin:  "testuser",
							UserAvatar: "/avatars/test.jpg",
							IsMine:     true,
						},
...
func TestGetUsersFavFilms(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "Success",
			context: testContextWithUsersUser(user),
			mockSetup: func() {
				...
				mockClient.EXPECT().
					GetFavFilms(gomock.Any(), &gen.GetFavFilmsRequest{
						UserId: userID.String(),
					}).
			...
			mockSetup: func() {
				...
				for _, film := range expectedFilms {
					grpcFilms = append(grpcFilms, &gen.FilmInCalendar{
						...
						ShortDescription: film.ShortDescription,
						ReleaseDate:      film.ReleaseDate.Format(TimeLayout),
...
func TestGetSimilarFilms(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			mockSetup: func() {
				...
				mockClient.EXPECT().
					GetSimilarFilms(gomock.Any(), &gen.GetSimilarFilmsRequest{
						FilmId: filmIDStr,
					}).
					Return(&gen.GetSimilarFilmsResponse{Films: grpcFilms}, nil)
			},
			expectedStatus: http.StatusOK,
			expectBody:     true,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			router := mux.NewRouter()
			router.HandleFunc("/films/{id}/similar", handler.GetSimilarFilms)
...
func TestGetUsersRecommendations(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			mockSetup: func() {
				...
				mockClient.EXPECT().
					GetUsersRecommendations(gomock.Any(), &gen.GetUsersRecommendationsRequest{
						UserId: userID.String(),
					}).
					Return(&gen.GetSimilarFilmsResponse{Films: grpcFilms}, nil)
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			router := mux.NewRouter()
			router.HandleFunc("/films/recommendations", handler.GetUsersRecommendations)
			router.ServeHTTP(rec, req)
